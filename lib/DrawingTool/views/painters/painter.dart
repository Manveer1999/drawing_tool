...
class Painter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (tempScale != null) {
      ...
      canvas.transform(Matrix4.identity()
          .scaled(size.width / tempScale.width, size.height / tempScale.height)
          .storage);
    }
    ...
    for (final drawable in drawables.where((drawable) => drawable.isNotHidden)) {
      drawable.draw(canvas, tempScale ?? size);
    }
    canvas.restore();
  ...
  bool shouldRepaint(covariant CustomPainter oldDelegate) {
    ...
    return oldDelegate.background != background ||
        !const ListEquality<Drawable>().equals(oldDelegate.drawables, drawables);
  }
}
