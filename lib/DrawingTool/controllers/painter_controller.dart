...
class DrawingToolController extends ValueNotifier<PainterControllerValue> {
  ...
  double angle = RotationAngles.degree0;

  double rotationScale = 1;

  static PainterSettings defaultPainterSettings(Paint shapePaint) => PainterSettings(
  ...
  /// Create a [DrawingToolController].
  ///
  ...
  /// Create a [DrawingToolController] from a [PainterControllerValue].
  DrawingToolController.fromValue(PainterControllerValue value)
  ...
  /// Setting this will notify all the listeners of this [DrawingToolController]
  ...
  Queue<ControllerAction<dynamic, dynamic>> performedActions = DoubleLinkedQueue(),
      unperformedActions = DoubleLinkedQueue();

  /// Uses the [PainterControllerWidget] inherited widget to fetch the [DrawingToolController] instance in this context.
  /// This is used internally in the library to fetch the controller at different widgets.
  static DrawingToolController of(BuildContext context) {
  ...
  ///
  /// Calling this will notify all the listeners of this [DrawingToolController]
  /// that they need to update (it calls [notifyListeners]). For this reason,
  ...
  void rotateCanvas(Size backgroundImage) {
    switch(quarterTurns) {
      case 0:
        angle = RotationAngles.degree90;
        quarterTurns++;
        rotationScale = backgroundImage.width/backgroundImage.height;
        break;
      case 1:
        angle = RotationAngles.degree180;
        quarterTurns++;
        rotationScale = 1;
        break;
      case 2:
        angle = RotationAngles.degree270;
        quarterTurns++;
        rotationScale = backgroundImage.width/backgroundImage.height;
        break;
      case 3:
        angle = RotationAngles.degree0;
        quarterTurns = 0;
        rotationScale = 1;
        break;
    }

    if(rotationScale < 0.55) {
      rotationScale = 0.55;
    }
  ...
  void _addAction(ControllerAction<dynamic, dynamic> action, bool newAction) {
  ...
  Future<ui.Image> renderImage(Size size) async {
    ...
    final tempSize = Size(size.width, size.height);
    ...
    painter.paint(canvas, tempSize);
    return await recorder
        .endRecording()
        .toImage(tempSize.width.floor(), tempSize.height.floor());
  }
...
class PainterControllerValue {
  ...
  bool operator ==(Object other) {
    return other is PainterControllerValue &&
        (const ListEquality<Drawable>().equals(_drawables, other._drawables) &&
...
class RotationAngles {
  static const double degree0 = 0;
  static const double degree90 = 3.14 / 2;
  static const double degree180 = 3.14;
  static const double degree270 = - 3.14 / 2;
}
